...
function buildPreview(excelRows) {
  const seen = new Map()
  const out = []
  for (let i = 0; i < excelRows.length; i++) {
    ...
    let email = getCell(row, 'email', 'e-mail', 'correo', 'correo electrónico', 'correo electronico')
    email = email.toLowerCase()

    const errParts = []
    if (!nombre) errParts.push('falta nombre')
    if (!apellido) errParts.push('falta apellido')
    if (!email) errParts.push('falta email')
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) errParts.push('email inválido')
    if (email && seen.has(email)) errParts.push('email duplicado en el archivo')
    if (email) seen.set(email, sheetRow)
...
/** Agrupa fallos por mensaje de API (ej. muchos "email: … unique"). */
...
export default function UserImportModal({ open, onClose, onImported, flash }) {
  ...
  const runImport = async () => {
    ...
    for (let i = 0; i < validRows.length; i++) {
      ...
      try {
        ...
        succeeded.push({ sheetRow: r.sheetRow, email: r.email, nombre: r.nombre, apellido: r.apellido })
      } catch (e) {
    ...
    if (failed.length === 0) {
      flash?.('success', `Se importaron ${succeeded.length} usuario(s) correctamente.`)
    } else if (succeeded.length > 0) {
      flash?.('error', `Importación parcial: ${succeeded.length} creados, ${failed.length} con error. Revisá el resumen.`)
    } else {
      flash?.('error', 'No se pudo crear ningún usuario. Revisá el resumen de errores.')
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
      <div className="relative max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-slate-200 bg-white p-6 shadow-xl dark:border-surface-700 dark:bg-surface-900">
        ...
        {importing && (
          ...
          >
            <LoadingSpinner />
            <p className="text-sm font-medium text-slate-800 dark:text-slate-100">Importando usuarios…</p>
            <p className="text-xs text-slate-500 dark:text-slate-400">Se están subiendo los registros al servidor</p>
        ...
        <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Importar usuarios desde Excel</h2>
        <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">
          La planilla debe tener columnas <strong className="font-medium text-slate-800 dark:text-slate-200">Nombre</strong>,{' '}
          <strong className="font-medium text-slate-800 dark:text-slate-200">Apellido</strong> y{' '}
          <strong className="font-medium text-slate-800 dark:text-slate-200">Email</strong> (primera fila = encabezados). El
          rol se asigna siempre como <code className="rounded bg-slate-100 px-1 font-mono text-xs dark:bg-surface-800">USER</code> y
          la contraseña inicial para todos es <code className="rounded bg-slate-100 px-1 font-mono text-xs dark:bg-surface-800">{BULK_DEFAULT_PASSWORD}</code>.
        </p>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          ...
          {fileLabel ? (
            <span className="text-sm text-slate-500 dark:text-slate-400">Archivo: {fileLabel}</span>
          ) : null}
        </div>
          ...
          <div className="mt-4 space-y-3">
            <div className="grid gap-2 rounded-xl border border-slate-200 bg-slate-50/80 p-4 text-sm dark:border-surface-600 dark:bg-surface-800/50">
              ...
              <ul className="space-y-1 text-slate-600 dark:text-slate-300">
                <li>
                  <span className="text-emerald-700 dark:text-emerald-400">Creados en el sistema:</span>{' '}
                  <strong>{importResult.created}</strong> de {importResult.totalAttempted} enviados
                </li>
                {importResult.skippedInPreview > 0 ? (
                  <li>
                    <span className="text-amber-700 dark:text-amber-300">Omitidos (datos inválidos en el archivo):</span>{' '}
                    <strong>{importResult.skippedInPreview}</strong> (no se enviaron al servidor)
                  </li>
                ) : null}
                {hasFailures ? (
                  <li>
                    <span className="text-red-700 dark:text-red-300">Rechazados por el servidor:</span>{' '}
            ...
            {hasSuccesses && (
              <div className="rounded-xl border border-emerald-200/80 bg-emerald-50/80 px-3 py-2 text-sm text-emerald-950 dark:border-emerald-900/30 dark:bg-emerald-950/25 dark:text-emerald-100">
                <p className="font-medium">Registros importados correctamente</p>
                  ...
                  <ul className="mt-2 max-h-40 list-inside list-disc space-y-0.5 overflow-y-auto text-xs font-mono">
                    {importResult.succeeded.map((s) => (
                      <li key={`ok-${s.sheetRow}-${s.email}`}>
                        Fila {s.sheetRow}: {s.nombre} {s.apellido} — {s.email}
                      </li>
                    ))}
                  </ul>
                </details>
            ...
            {hasFailures && (
              <div className="rounded-xl border border-amber-200 bg-amber-50/90 px-3 py-2 text-sm text-amber-950 dark:border-amber-900/40 dark:bg-amber-950/35 dark:text-amber-100">
                <p className="font-medium">Errores al crear (con motivo)</p>
                <p className="mt-0.5 text-xs opacity-90">
                  Suele deberse a un <strong>email ya registrado</strong>, datos que no pasan la validación, etc.
                </p>
        ...
        {!showResult && preview.length > 0 ? (
          <div className="mt-4">
            <p className="text-sm text-slate-600 dark:text-slate-400">
              Vista prevía: {validRows.length} fila(s) lista(s) para importar
              ...
              <table className="w-full text-left text-xs">
                <thead className="bg-slate-50 dark:bg-surface-800/90">
                  <tr>
                    ...
                    <th className="px-2 py-2 font-medium text-slate-500">Nombre</th>
                    <th className="px-2 py-2 font-medium text-slate-500">Apellido</th>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-surface-700">
                  {preview.map((r) => (
                    <tr key={r.sheetRow} className={r.error ? 'bg-red-50/80 dark:bg-red-950/20' : ''}>
                      ...
                      <td className="px-2 py-1.5 text-slate-800 dark:text-slate-200">{r.nombre || '—'}</td>
                      <td className="px-2 py-1.5 text-slate-800 dark:text-slate-200">{r.apellido || '—'}</td>
